...
export class CategoryService {
  private apiUrl = 'http://localhost:8000/api/v1/categories/'; // غيّرها حسب الـ API الحقيقي
  ...
  deleteCategory(id: string): Observable<void> {
    return this.http.delete<void>(`http://localhost:8000/api/v1/categories/${id}`);
  }
  updateCategory(id: string, newName: string): Observable<Categoryinfo> {
    return this.http.put<Categoryinfo>(`http://localhost:8000/api/v1/categories/${id}`, { name: newName });
  }
  apiproducturl='http://localhost:8000/api/v1/product/'
  ...
  deleteproduct(id: string): Observable<void> {
    return this.http.delete<void>(`http://localhost:8000/api/v1/product/${id}`);
  }
  updateproduct(id: string, newName: string): Observable<Productinfo> {
    return this.http.put<Productinfo>(`http://localhost:8000/api/v1/product/${id}`, { name: newName });
  }
  ...
  sendContactForm(contactData: Contact): Observable<any> {
    return this.http.post<any>('http://localhost:8000/api/v1/submit/'
      , contactData);
  }
  getContactForm(): Observable<any> {
    return this.http.get<any>('http://localhost:8000/api/v1/submit/');
  }
  deleteresponse(id: string): Observable<void> {
    return this.http.delete<void>(`http://localhost:8000/api/v1/submit/${id}`);
  }
  sendorderForm(orderData: Order): Observable<any> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    return this.http.post<any>('http://localhost:8000/api/v1/order/', JSON.stringify(orderData), { headers });
  }
  getorderForm(): Observable<any> {
    return this.http.get<any>('http://localhost:8000/api/v1/order/');
  }
  deleteorder(id: string): Observable<void> {
    return this.http.delete<void>(`http://localhost:8000/api/v1/order/${id}`);
  }
}
